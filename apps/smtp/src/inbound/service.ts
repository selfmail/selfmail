...
export abstract class Inbound {
	...
	static async saveMailToS3(mail: ParsedMail) {
		...
		const headersObj: Record<string, string | string[] | { prepared: boolean; value: string }> = {};
		const toHeaderValue = (v: unknown): string | string[] | { prepared: boolean; value: string } => {
			if (typeof v === "string") return v;
			if (Array.isArray(v) && (v as unknown[]).every((x) => typeof x === "string")) return v as string[];
		...
		if (mail.headers instanceof Map) {
			...
			for (const [key, value] of Object.entries(mail.headers as Record<string, unknown>)) {
				headersObj[key] = toHeaderValue(value);
			}
		}
		...
		const composerOptions = {
			...
			attachments: mail.attachments?.map((att) => ({
				...
				cid: att.cid,
				contentDisposition:
					att.contentDisposition === "attachment" || att.contentDisposition === "inline"
		...
		try {
			...
			const savedEmail = await db.email.create({
				data: {
					...
					cc: mail.cc ? JSON.parse(JSON.stringify(extractAddresses(mail.cc))) : null,
					bcc: mail.bcc ? JSON.parse(JSON.stringify(extractAddresses(mail.bcc))) : null,
					replyTo: mail.replyTo ? JSON.parse(JSON.stringify(extractAddresses(mail.replyTo))) : null,
					text: mail.text || null,
					html: mail.html || null,
					headers: headersToJson(mail.headers) ? JSON.parse(JSON.stringify(headersToJson(mail.headers))) : null,
					attachments: mail.attachments?.map((att) => ({
						filename: att.filename,
						contentType: att.contentType,
						size: att.size,
