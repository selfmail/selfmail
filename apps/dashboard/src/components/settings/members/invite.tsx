...
const roleOptions = [
	{
		description: "dashboard.settings.member_settings.role_member_description",
		label: "dashboard.settings.member_settings.role_member_label",
		value: "member",
	},
	{
		description: "dashboard.settings.member_settings.role_viewer_description",
		label: "dashboard.settings.member_settings.role_viewer_label",
		value: "viewer",
	},
] as const;
...
export default function InviteMemberMenu({
	...
	const invite = useMutation({
		...
		onSuccess: () => {
			toast.success(m["dashboard.settings.member_settings.invite_sent"](), {
				description: m[
					"dashboard.settings.member_settings.invite_sent_description"
				]({ email }),
		...
		onError: (error) => {
			console.log(error);
			toast.error(m["dashboard.settings.member_settings.invite_failed"](), {
				description:
					error instanceof Error
						? error.message
						: m["dashboard.settings.member_settings.unexpected_error"](),
				id: "invite-member-api",
			});
		},
	});
	...
	function handleSubmit() {
		...
		if (!parse.success) {
			setError(m["dashboard.settings.member_settings.form_error"]());
			return;
		}
							...
							<div className="grid gap-4">
								...
								<div className="grid gap-2">
									<Label htmlFor={roleId}>
										{m["dashboard.settings.member_settings.role"]()}
									</Label>
									<Select
										onValueChange={(value) =>
									...
									>
										<SelectTrigger id={roleId}>
											<SelectValue
												placeholder={m[
													"dashboard.settings.member_settings.select_role"
												]()}
											/>
										</SelectTrigger>
										<SelectContent>
									...
									<p className="text-pretty text-muted-foreground text-sm">
										{m[selectedRole.description]()}
									</p>
								</div>

								<div className="grid gap-2">
									<Label htmlFor={noteId}>
										{m["dashboard.settings.member_settings.message"]()}
									</Label>
									<Textarea
										id={noteId}
										maxLength={180}
										...
										placeholder={m[
											"dashboard.settings.member_settings.message_placeholder"
										]()}
										value={note}
									/>
