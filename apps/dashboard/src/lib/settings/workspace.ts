...
export const updateWorkspace = createServerFn({ method: "POST" })
	...
	.handler(
		async ({
			...
			const currentWorkspace = await db.workspace.findUniqueOrThrow({
				where: { id: workspaceId },
				select: { image: true },
			});
			let imageUrl = currentWorkspace.image;
			if (image !== undefined && image !== currentWorkspace.image) {
				imageUrl = null;
				if (image) {
					const { uploadWorkspaceImage } = await import(
						"./workspace-images.server"
					);
					imageUrl = await uploadWorkspaceImage(workspaceId, image);
				}
			}
