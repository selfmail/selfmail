...
function getSenderLabel(value: unknown) {
	...
	return name || address || m["dashboard.email.unknown_sender"]();
}
...
function formatInboxDate(date: Date) {
	...
	if (diff < hour) {
		return m["dashboard.email.minute_ago"]({
			count: Math.max(1, Math.floor(diff / minute)),
		});
	}

	if (diff < day) {
		return m["dashboard.email.hour_ago"]({ count: Math.floor(diff / hour) });
	}

	if (diff < 7 * day) {
		return m["dashboard.email.day_ago"]({ count: Math.floor(diff / day) });
	}
...
export function toDashboardEmail(email: {
	...
	const snippet = email.text?.trim() || m["dashboard.email.no_preview"]();
