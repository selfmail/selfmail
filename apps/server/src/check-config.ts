...
export default function CheckConfig() {
    ...
    if (!config.RESEND || config.RESEND === "") throw new Error("RESEND Key is not defined") 
    if (!config.SUPPORT_MAIL || config.SUPPORT_MAIL === "" as `${string}@${string}.${string}`) throw new Error("SUPPORT_MAIL is not defined.")
}