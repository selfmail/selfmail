...
const hexColorPattern = /^#(0x)?[0-9a-f]+$/i;
const rgbColorPattern =
  /^rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/;
const cssSizePattern = /^\d+(?:px|em|%|pt)$/;
const fontWeightPattern = /^(?:normal|bold|bolder|lighter|[1-9]00)$/;
const fontStylePattern = /^(?:normal|italic|oblique)$/;
const anyValuePattern = /.*/;
const textAlignPattern = /^(?:left|right|center|justify)$/;
const textDecorationPattern = /^(?:none|underline|overline|line-through)$/;
const boxSpacingPattern = /^\d+(?:px|em|%|pt)(?:\s+\d+(?:px|em|%|pt)){0,3}$/;
...
export abstract class Spam {
  ...
  static createProperAttachments(
    attachments: {
  ...
  static async check(emailData: ParsedMail): Promise<{
    ...
    if (emailData.attachments && emailData.attachments.length > 0) {
      const attachments = Spam.createProperAttachments(
    ...
    if (emailData.html) {
      const sanitizedHtml = Spam.sanitizeHtml(emailData.html);
    ...
    const sanitizedHtml = sanitize(html, {
      ...
      allowedStyles: {
        "*": {
          ...
          "font-size": [cssSizePattern],
          "font-weight": [fontWeightPattern],
          "font-style": [fontStylePattern],
          "font-family": [anyValuePattern],
          "text-align": [textAlignPattern],
          "text-decoration": [textDecorationPattern],

          // Layout
          margin: [boxSpacingPattern],
          padding: [boxSpacingPattern],
          border: [anyValuePattern],
          width: [cssSizePattern],
          height: [cssSizePattern],
        },
      },
