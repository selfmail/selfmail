...
function DialogProvider({ children }: { children: React.ReactNode }) {
  const { open, content, setContent, setOpenId, setOpen, classNames } =
    useDialogStore();
  return (
    <>
      {children}
      ...
      {open && content && (
        <>
          <div className="w-full h-screen flex items-center justify-center fixed z-40 top-0">
            <div
              className="w-full h-screen absolute bg-[#00000070] backdrop-brightness-75 "
              onClick={() => {
            ...
            <div
              className={cn(
                "absolute z-50 text-black bg-[#f4f4f4] lg:w-[500px] rounded-lg p-2",
                classNames,
              )}
            >
              {content}
...
function Dialog({ children }: { children: React.ReactNode }) {
  return (
    <IdContext.Provider value={Math.random()}>{children}</IdContext.Provider>
  );
}
